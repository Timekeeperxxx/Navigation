...
规划静态点云由 C++/PCL 发布器逐层加载，并按 `map=0.2m`、`ground=0.3m`、`footprint_fill=0.1m` 在内存中体素化；原始 `map.pcd` 保持不变并继续供重定位使用。启动脚本不再按固定文件大小或点数拒绝地图，而是根据 PCD 点数、当前 `MemAvailable` 和默认 4 GiB 系统/视频预留执行动态内存预检。
