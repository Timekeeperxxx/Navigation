...
namespace scan_planner
{
  ...
  void SCANReplanFSM::init(const rclcpp::Node::SharedPtr &nh)
  {
    ...
    emergency_enter_time_ = node_->now();
    last_emergency_replan_time_ = node_->now();
    ...
    getParam(nh, "fsm/final_goal_tolerance", final_goal_tolerance_, 0.5);
    final_goal_tolerance_ = std::max(0.05, final_goal_tolerance_);
    getParam(nh, "fsm/emergency_time_", emergency_time_, 1.0);
    getParam(nh, "fsm/emergency_min_stop_duration", emergency_min_stop_duration_, 0.5);
    emergency_min_stop_duration_ = std::max(0.0, emergency_min_stop_duration_);
    getParam(nh, "fsm/emergency_replan_period", emergency_replan_period_, 1.0);
    emergency_replan_period_ = std::max(0.1, emergency_replan_period_);
    ...
    /* callback */
    exec_timer_ = nh->create_wall_timer(std::chrono::milliseconds(20), std::bind(&SCANReplanFSM::execFSMCallback, this));
    safety_timer_ = nh->create_wall_timer(std::chrono::milliseconds(20), std::bind(&SCANReplanFSM::checkCollisionCallback, this));
  ...
  void SCANReplanFSM::pathCallback(const nav_msgs::msg::Path::ConstSharedPtr &msg)
  {
    ...
    trigger_ = true;
    const auto &raw_start = msg->poses.front().pose.position;
    const auto &raw_end = msg->poses.back().pose.position;
    ...
    ROS_WARN("[SCAN path] received /initial_path: points=%zu frame=%s "
             "raw_start=(%.3f, %.3f, %.3f) raw_end=(%.3f, %.3f, %.3f) "
             "body_start=(%.3f, %.3f, %.3f) body_end=(%.3f, %.3f, %.3f) "
             "state=%d have_odom=%d have_target=%d trigger=%d replan_fail=%d.",
             msg->poses.size(),
             msg->header.frame_id.c_str(),
             raw_start.x, raw_start.y, raw_start.z,
             raw_end.x, raw_end.y, raw_end.z,
             waypoints.front()(0), waypoints.front()(1), waypoints.front()(2),
    ...
    if (success)
    {
      ...
      ROS_WARN("[SCAN path] accepted /initial_path: state=%d have_target=%d have_new_target=%d "
               "end_pt=(%.3f, %.3f, %.3f) active_waypoints=%zu.",
               static_cast<int>(exec_state_),
               have_target_,
               have_new_target_,
  ...
  void SCANReplanFSM::publishSelfInflationMarker()
  {
    ...
    if (std::abs(self_double_cylinder_offset_) <= 1e-6)
      return;
  ...
  void SCANReplanFSM::changeFSMExecState(FSM_EXEC_STATE new_state, string pos_call)
  {
    ...
    if (new_state == EMERGENCY_STOP && exec_state_ != EMERGENCY_STOP)
    {
      flag_escape_emergency_ = true;
      emergency_enter_time_ = node_->now();
      last_emergency_replan_time_ = emergency_enter_time_;
    }
  ...
  void SCANReplanFSM::execFSMCallback()
  {
    ...
    {
      printFSMExecState();
      ROS_WARN("[SCAN FSM diag] state=%d have_odom=%d trigger=%d have_target=%d have_new_target=%d "
               "active_waypoints=%zu replan_fail=%d go2_frozen=%d odom=(%.3f, %.3f, %.3f) "
               "end=(%.3f, %.3f, %.3f).",
               static_cast<int>(exec_state_),
               have_odom_,
               trigger_,
    ...
    case GEN_NEW_TRAJ:
    {
      ...
      setStartStateFromOdomOrCurrentTraj();
      if (currentPlanningStartOccupied("new trajectory"))
      {
        replan_fail_count_++;
        flag_escape_emergency_ = true;
        changeFSMExecState(EMERGENCY_STOP, "START_OCCUPIED");
        break;
      }
    ...
    case REPLAN_TRAJ:
    {
      ...
      if (currentPlanningStartOccupied("replan"))
      {
        replan_fail_count_++;
        flag_escape_emergency_ = true;
        changeFSMExecState(EMERGENCY_STOP, "START_OCCUPIED");
    ...
    case EMERGENCY_STOP:
    {
      ...
      if (flag_escape_emergency_) // Avoiding repeated calls
      {
        ...
        emergency_enter_time_ = now;
        last_emergency_replan_time_ = now;
      }
      else
      {
        ...
        else if (enable_fail_safe_ && have_target_ && odom_vel_.norm() < 0.15 &&
                 (now - emergency_enter_time_).seconds() >= emergency_min_stop_duration_ &&
                 (now - last_emergency_replan_time_).seconds() >= emergency_replan_period_)
        {
          last_emergency_replan_time_ = now;
          ROS_WARN("[emergency recovery] Try frozen-start replan after %.2fs stop.",
                   (now - emergency_enter_time_).seconds());
          if (currentPlanningStartOccupied("emergency recovery"))
            break;
          ...
          if (planFromCurrentTraj())
          {
            replan_fail_count_ = 0;
            flag_escape_emergency_ = true;
            changeFSMExecState(EXEC_TRAJ, "EMERGENCY_RECOVER");
          }
          else
          {
            replan_fail_count_++;
            ROS_WARN("[emergency recovery] Replan failed; stay in EMERGENCY_STOP and retry later. fail_count=%d",
                     replan_fail_count_);
          }
        }
      }
  ...
  bool SCANReplanFSM::planFromCurrentTraj()
  {
    ...
    // Periodic replanning normally inherits the previous optimized trajectory.
    // When execution is frozen, or that inherited point has already moved away
    // from the real planning start, initialize from the current pose instead so
    // the published execution path remains spatially attached to the robot.
    constexpr double detached_replan_xy_threshold = 0.50;
    bool force_poly_init = go2_execution_frozen_;
    Eigen::Vector3d inherited_start = start_pt_;
    double inherited_start_xy_error = 0.0;
    if (info->start_time_.seconds() > 1e-5 && info->duration_ > 1e-5)
    {
      inherited_start = info->position_traj_.evaluateDeBoorT(t_cur);
      inherited_start_xy_error = (inherited_start - start_pt_).head<2>().norm();
      if (inherited_start_xy_error > detached_replan_xy_threshold)
        force_poly_init = true;
    }

    if (force_poly_init)
    {
      ROS_WARN("[SCAN replan] Force polynomial init for attached execution path: "
               "go2_frozen=%d inherited_start=(%.3f, %.3f, %.3f) "
               "planning_start=(%.3f, %.3f, %.3f) xy_error=%.3f threshold=%.3f.",
               go2_execution_frozen_,
               inherited_start(0), inherited_start(1), inherited_start(2),
               start_pt_(0), start_pt_(1), start_pt_(2),
               inherited_start_xy_error,
               detached_replan_xy_threshold);
    }

    bool success = callReboundReplan(force_poly_init, false);
    if (!success && !force_poly_init)
    {
  ...
  bool SCANReplanFSM::currentPlanningStartOccupied(const char *context)
  {
    auto map = planner_manager_->grid_map_;
    if (!map)
      return false;
    ...
    const Eigen::Vector3d planning_start = getPlanningStartPosition();
    const double yaw = estimateYawFromSegment(planning_start, end_pt_);
    const int occ = map->getInflateOccupancy(planning_start, yaw);
    if (occ == 0)
      return false;

    ROS_WARN("[%s] Current planning start is not free; hold stop and wait for clearance/new path. "
             "region=%s occ=%d start=(%.3f, %.3f, %.3f), yaw=%.3f, "
             "odom=(%.3f, %.3f, %.3f), end=(%.3f, %.3f, %.3f), start_to_end_xy=%.3f",
             context ? context : "replan",
             occ < 0 ? "start_out_of_map" : "start_occupied",
             occ,
             planning_start(0), planning_start(1), planning_start(2), yaw,
             odom_pos_(0), odom_pos_(1), odom_pos_(2),
             end_pt_(0), end_pt_(1), end_pt_(2),
             (planning_start - end_pt_).head<2>().norm());
    return true;
  }
  ...
  void SCANReplanFSM::checkCollisionCallback()
  {
    ...
    if (exec_state_ == WAIT_TARGET || exec_state_ == EMERGENCY_STOP || info->start_time_.seconds() < 1e-5)
      return;

    /* ---------- check trajectory ---------- */
    constexpr double time_step = 0.01;
    constexpr double near_start_diag_radius = 0.30;
    double t_cur = (node_->now() - info->start_time_).seconds();
    const Eigen::Vector3d traj_start = info->position_traj_.evaluateDeBoorT(0.0);
    for (double t = t_cur; t < info->duration_; t += time_step)
    {
      ...
      const double yaw = estimateYawFromSegment(pos, pos_next);
      const int occ = map->getInflateOccupancy(pos, yaw);
      if (occ != 0)
      {
        const double hit_to_odom_xy = (pos - odom_pos_).head<2>().norm();
        const double hit_to_traj_start_xy = (pos - traj_start).head<2>().norm();
        const char *region = occ < 0 ? "path_out_of_map"
                                     : (hit_to_odom_xy <= near_start_diag_radius
                                            ? "near_robot"
                                            : (hit_to_traj_start_xy <= near_start_diag_radius ? "near_traj_start"
                                                                                              : "path_ahead"));
        ROS_WARN("[SCAN safety] Active trajectory collision detected: "
                 "state=%d traj_id=%d t_cur=%.3f t_hit=%.3f dt_hit=%.3f duration=%.3f "
                 "region=%s occ=%d hit_to_odom_xy=%.3f hit_to_traj_start_xy=%.3f "
                 "near_start_diag_radius=%.3f pos=(%.3f, %.3f, %.3f) "
                 "next=(%.3f, %.3f, %.3f) yaw=%.3f odom=(%.3f, %.3f, %.3f) "
                 "traj_start=(%.3f, %.3f, %.3f) go2_frozen=%d.",
                 static_cast<int>(exec_state_),
                 info->traj_id_,
                 t_cur,
                 t,
                 ...
                 occ,
                 hit_to_odom_xy,
                 hit_to_traj_start_xy,
                 near_start_diag_radius,
                 pos(0), pos(1), pos(2),
                 pos_next(0), pos_next(1), pos_next(2),
                 yaw,
                 odom_pos_(0), odom_pos_(1), odom_pos_(2),
                 traj_start(0), traj_start(1), traj_start(2),
                 go2_execution_frozen_);
  ...
  bool SCANReplanFSM::callReboundReplan(bool flag_use_poly_init, bool flag_randomPolyTraj)
  {
    ...
    const rclcpp::Time t_target_start = t_total_start;
    getLocalTarget();
    const rclcpp::Duration t_target = node_->now() - t_target_start;

    const bool use_poly_init = have_new_target_ || flag_use_poly_init;
    const rclcpp::Time t_replan_start = node_->now();
    bool plan_success =
        planner_manager_->reboundReplan(start_pt_, start_vel_, start_acc_, local_target_pt_, local_target_vel_, use_poly_init, flag_randomPolyTraj);
    const rclcpp::Duration t_replan = node_->now() - t_replan_start;
    have_new_target_ = false;

    cout << "final_plan_success=" << plan_success << endl;
    RCLCPP_INFO(
        node_->get_logger(),
        "[SCAN timing] callReboundReplan result=%d total=%.3f target_select=%.3f rebound=%.3f "
        "state=%d start=(%.2f,%.2f,%.2f) local_target=(%.2f,%.2f,%.2f) "
        "use_poly_init=%d random_poly=%d go2_frozen=%d.",
        plan_success,
    ...
    if (plan_success)
    {
      ...
      ROS_INFO("[SCAN publish] Rebound trajectory accepted: traj_id=%d duration=%.3f "
               "start=(%.3f, %.3f, %.3f) end=(%.3f, %.3f, %.3f) "
               "local_target=(%.3f, %.3f, %.3f) start_vel=(%.3f, %.3f, %.3f) "
               "go2_frozen=%d poly_init=%d random_poly=%d.",
               info->traj_id_,
               info->duration_,
               traj_start(0), traj_start(1), traj_start(2),
               traj_end(0), traj_end(1), traj_end(2),
               local_target_pt_(0), local_target_pt_(1), local_target_pt_(2),
      ...
      ROS_WARN("[SCAN publish] Published /planning/bspline: traj_id=%ld points=%zu knots=%zu "
               "duration=%.3f subscribers=%zu.",
               static_cast<long>(bspline.traj_id),
               bspline.pos_pts.size(),
               bspline.knots.size(),
               info->duration_,
               bspline_pub_->get_subscription_count());
  ...
  bool SCANReplanFSM::callEmergencyStop(Eigen::Vector3d stop_pos)
  {
    ...
    auto info = &planner_manager_->local_data_;
    ROS_WARN("[SCAN emergency] Publish emergency stop trajectory: traj_id=%d stop_pos=(%.3f, %.3f, %.3f).",
  ...
  bool SCANReplanFSM::getReferencePathLocalTarget()
  {
    ...
    int local_target_occupancy = targetOccupancy(local_target_pt_, target_segment);

    if (local_target_occupancy != 0)
    {
      bool found_free_target = false;
      double forward_distance = 0.0;
      for (size_t i = target_segment; i < segment_count; ++i)
      {
        const Eigen::Vector3d segment = active_waypoints_[i + 1] - active_waypoints_[i];
        const double length = segment.head<2>().norm();
        if (length < 1e-4)
        ...
        const double from_ratio = i == target_segment ? target_ratio : 0.0;
        const double sample_step = std::max(0.10, planner_manager_->grid_map_->getResolution());
        for (double distance_on_segment = 0.0;
             distance_on_segment <= (1.0 - from_ratio) * length + 1e-6;
             distance_on_segment += sample_step)
        {
          if (forward_distance + distance_on_segment > planning_horizon_)
            break;

          const double ratio = std::min(1.0, from_ratio + distance_on_segment / length);
          const Eigen::Vector3d candidate = targetFromSegment(i, ratio);
          if ((candidate - start_pt_).head<2>().norm() < 0.4)
            continue;
          if (targetOccupancy(candidate, i) == 0)
          {
            local_target_pt_ = candidate;
            target_segment = i;
            target_ratio = ratio;
            reached_end = i == segment_count - 1 && ratio > 1.0 - 1e-3;
            found_free_target = true;
            local_target_occupancy = 0;
            RCLCPP_WARN_THROTTLE(
                node_->get_logger(), *node_->get_clock(), 1000,
                "[reference path] Lookahead target occupied; use forward collision-free point on original path.");
            break;
          }
        }

        if (found_free_target || forward_distance > planning_horizon_)
          break;

        forward_distance += (1.0 - from_ratio) * length;
      }
      ...
      if (!found_free_target)
      {
        for (size_t i = target_segment; i < segment_count; ++i)
        {
          const Eigen::Vector3d &candidate = active_waypoints_[i + 1];
          if ((candidate - start_pt_).head<2>().norm() < 0.4)
            continue;
          if (targetOccupancy(candidate, i) == 0)
          {
            ...
            target_segment = i;
            target_ratio = 1.0;
            reached_end = i == segment_count - 1;
            found_free_target = true;
            local_target_occupancy = 0;
            RCLCPP_WARN_THROTTLE(
                node_->get_logger(), *node_->get_clock(), 1000,
                "[reference path] Lookahead target occupied; use forward waypoint after occupied segment.");
            break;
          }
        }
      }
      ...
      if (!found_free_target)
      {
        RCLCPP_WARN_THROTTLE(
            node_->get_logger(), *node_->get_clock(), 1000,
            "[reference path] No forward collision-free lookahead point found; keep bounded target for fail-safe handling.");
      }
    }
    ...
    RCLCPP_INFO_THROTTLE(
        ...
        "[reference path] progress=%zu+%.2f target=%zu+%.2f xyz=(%.2f, %.2f, %.2f) "
        "horizon=%.2f robot_to_ref=%.3f target_occ=%d target_to_final=%.3f waypoints=%zu",
        ...
        std::sqrt(best_distance_squared), local_target_occupancy,
        (local_target_pt_ - active_waypoints_.back()).head<2>().norm(), active_waypoints_.size());
    return true;
  }
  ...
  void SCANReplanFSM::getLocalTarget()
  {
    ...
    if (getReferencePathLocalTarget())
    {
      RCLCPP_INFO_THROTTLE(
          node_->get_logger(), *node_->get_clock(), 1000,
          "[SCAN timing] getLocalTarget source=reference_path total=%.3f target=(%.2f,%.2f,%.2f).",
          (node_->now() - t_total_start).seconds(),
          local_target_pt_(0), local_target_pt_(1), local_target_pt_(2));
      return;
    }
      ...
      else
      {
        ...
        for (auto it = active_waypoints_.rbegin(); it != active_waypoints_.rend(); ++it)
        {
          if (isBoundedCandidate(*it) && targetOccupancy(*it) == 0)
          {
            ...
            RCLCPP_INFO_THROTTLE(
                node_->get_logger(), *node_->get_clock(), 1000,
                "[SCAN timing] getLocalTarget source=global_traj total=%.3f fallback=bounded_reference_point "
                "target=(%.2f,%.2f,%.2f).",
                (node_->now() - t_total_start).seconds(),
                local_target_pt_(0), local_target_pt_(1), local_target_pt_(2));
            return;
    ...
    RCLCPP_INFO_THROTTLE(
        node_->get_logger(), *node_->get_clock(), 1000,
        "[SCAN timing] getLocalTarget source=global_traj total=%.3f target_t=%.3f "
        "target=(%.2f,%.2f,%.2f) target_vel_norm=%.3f.",
        (node_->now() - t_total_start).seconds(),
        target_t,
        local_target_pt_(0), local_target_pt_(1), local_target_pt_(2),
