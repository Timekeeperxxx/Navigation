...
bool decodeDepthImage(const sensor_msgs::msg::Image& image, double scaling_factor,
                      std::vector<uint16_t>& output, std::string& error)
{
  const bool is_u16 = image.encoding == sensor_msgs::image_encodings::TYPE_16UC1 ||
                      image.encoding == sensor_msgs::image_encodings::MONO16;
  const bool is_f32 = image.encoding == sensor_msgs::image_encodings::TYPE_32FC1;
  if (!is_u16 && !is_f32)
  {
  ...
  {
    error = "depth image has zero width or height";
    return false;
  }
  if (is_f32 && (!std::isfinite(scaling_factor) || scaling_factor <= 0.0))
  {
    error = "invalid depth scaling factor";
    return false;
  }
  ...
  const size_t required_size = static_cast<size_t>(image.step) * image.height;
  if (image.step < min_step || image.data.size() < required_size)
  {
    error = "depth image step/data size is inconsistent with its dimensions";
    return false;
  }

  output.resize(static_cast<size_t>(image.width) * image.height);
  for (uint32_t row = 0; row < image.height; ++row)
  {
    const uint8_t* row_data = image.data.data() + static_cast<size_t>(row) * image.step;
    for (uint32_t col = 0; col < image.width; ++col)
    {
      const uint8_t* pixel = row_data + static_cast<size_t>(col) * bytes_per_pixel;
      uint16_t raw_depth = 0;
      if (is_u16)
      {
        raw_depth = image.is_bigendian
                        ? static_cast<uint16_t>((static_cast<uint16_t>(pixel[0]) << 8U) | pixel[1])
                        : static_cast<uint16_t>(pixel[0] | (static_cast<uint16_t>(pixel[1]) << 8U));
      }
      else
      {
        const uint32_t bits = image.is_bigendian
        ...
        float depth_m = 0.0F;
        std::memcpy(&depth_m, &bits, sizeof(depth_m));
        if (std::isfinite(depth_m) && depth_m > 0.0F)
        {
          const double scaled = static_cast<double>(depth_m) * scaling_factor;
          raw_depth = scaled >= std::numeric_limits<uint16_t>::max()
                          ? std::numeric_limits<uint16_t>::max()
                          : static_cast<uint16_t>(std::lround(scaled));
        }
      }
      output[static_cast<size_t>(row) * image.width + col] = raw_depth;
    }
  }
  return true;
}
...
void GridMap::initMap(const rclcpp::Node::SharedPtr& nh)
{
  ...
  getParam(node_, "grid_map/resolution", mp_.resolution_, -1.0);
  getParam(node_, "grid_map/sliding_map_size_x", x_size, -1.0);
  getParam(node_, "grid_map/sliding_map_size_y", y_size, -1.0);
  getParam(node_, "grid_map/sliding_map_size_z", z_size, -1.0);
  getParam(node_, "grid_map/local_update_range_x", mp_.local_update_range_(0), x_size/2.0);
  getParam(node_, "grid_map/local_update_range_y", mp_.local_update_range_(1), y_size/2.0);
  getParam(node_, "grid_map/local_update_range_z", mp_.local_update_range_(2), z_size/2.0);
  ...
  getParam(node_, "grid_map/fx", mp_.fx_, -1.0);
  getParam(node_, "grid_map/fy", mp_.fy_, -1.0);
  getParam(node_, "grid_map/cx", mp_.cx_, -1.0);
  getParam(node_, "grid_map/cy", mp_.cy_, -1.0);

  getParam(node_, "grid_map/depth_filter_maxdist", mp_.depth_filter_maxdist_, -1.0);
  getParam(node_, "grid_map/depth_filter_mindist", mp_.depth_filter_mindist_, -1.0);
  getParam(node_, "grid_map/depth_filter_margin", mp_.depth_filter_margin_, -1);
  getParam(node_, "grid_map/k_depth_scaling_factor", mp_.k_depth_scaling_factor_, -1.0);
  getParam(node_, "grid_map/skip_pixel", mp_.skip_pixel_, -1);

  getParam(node_, "grid_map/p_hit", mp_.p_hit_, -1.0);
  getParam(node_, "grid_map/p_miss", mp_.p_miss_, -1.0);
  getParam(node_, "grid_map/p_min", mp_.p_min_, -1.0);
  ...
  getParam(node_, "grid_map/ground_height", mp_.ground_height_, 0.0);
  ...
  if (mp_.sensor_type_ == "depth")
  {
    ...
    depth_pose_sub_.reset(
        new message_filters::Subscriber<nav_msgs::msg::Odometry>(node_, "/grid_map/sensor_pose"));
...
void GridMap::projectDepthImage()
{
  ...
  const int cols = static_cast<int>(md_.depth_image_width_);
  const int rows = static_cast<int>(md_.depth_image_height_);
  if (cols <= 0 || rows <= 0 || md_.depth_image_.empty())
    return;
  if (md_.proj_points_.size() < md_.depth_image_.size())
    md_.proj_points_.resize(md_.depth_image_.size());
  ...
  for (int v = mp_.depth_filter_margin_; v < rows - mp_.depth_filter_margin_; v += mp_.skip_pixel_)
  {
    ...
    {
      const uint16_t raw_depth = md_.depth_image_[static_cast<size_t>(v) * cols + u];
                                ...
                                const nav_msgs::msg::Odometry::ConstSharedPtr &pose)
{
  ...
  if (!decodeDepthImage(*img, mp_.k_depth_scaling_factor_, depth_image, decode_error))
  {
    ROS_ERROR("[GridMap] failed to decode depth image: %s", decode_error.c_str());
    return;
  }
  md_.depth_image_ = std::move(depth_image);
  md_.depth_image_width_ = img->width;
  md_.depth_image_height_ = img->height;
